...
exports.pictoaccessFestival = async (req, res) => {
    try {
        const festivalId = req.params.id;
        const { valid } = req.body; // true ou false attendu
        ...
        festival.pictoaccess = valid;
        await festival.save();

        res.status(200).json({ message: `Pictoaccess ${valid ? "validé" : "invalidé"} avec succès !` });
    } catch (err) {
        console.error("Erreur lors de la mise à jour de Pictoaccess :", err);
        res.status(500).json({ error: "Erreur serveur lors de la mise à jour de Pictoaccess." });
    }
};